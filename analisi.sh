...
function vs_plot {
	...
	echo -e "#ord\t\tvv_us\t\tv_us\t\tv_ss\t\ts_ss\t\tv_en\t\ts_en" > var_sdom.dat
	...
	do
		...
		then
			...
			mv --verbose --update ac.dat ac_${k}.dat
		fi
...
function move {
	...
	# sposto il file
	mv --verbose --update {var,sdom}.jpeg data.dat ${OUT} --target-directory=${DIR}
	# sposto dati e grafici degli autocorrelatori
	for ext in jpeg dat
	do
		for (( i = 1; i <= 30; i ++))
		do
			mv --verbose --update ac_${i}.${ext} --target-directory=${DIR}
		done
	done
}
